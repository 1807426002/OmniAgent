...
内容脚本捕获网页文件输入框的选择事件。对 DOCX、PDF、TXT：
...
| 情况 | 自动模式 | 确认模式 | 关闭模式 |
| --- | --- | --- | --- |
| 用户明确要求保存聊天内容 | 通过一次 `memory.save_batch` 直接写 Fact，不逐批确认 | 合法条目进入 Candidate | 拒绝自动写入 |
| 模型在普通聊天中主动建议保存 | 合法条目进入 Candidate | 合法条目进入 Candidate | 拒绝自动写入 |
| 用户在侧边栏手动添加、编辑或确认 | 直接执行人工操作 | 直接执行人工操作 | 直接执行人工操作 |

“明确要求”由扩展根据最近一条用户原话判断，模型不能自行声明。明确指令包括“记住”“保存到记忆”“写入记忆”“全部保存”等。自动模式下，模型必须停止普通回复，只调用一次批量工具；工具完成后不得再次调用、不得分批要求确认，只报告一次结果。

旧的“触发后 30 分钟自动抓取助手回复”以及固定字符截断逻辑已经移除。聊天也不再由内容脚本用简单关键词直接写入；普通聊天事实必须经过模型结构化提取、原文校验和候选策略。
...
| 自动模式仍出现候选 | 当前用户是否明确要求保存；普通模型主动保存必须进入候选 |
